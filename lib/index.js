...
function readFiles(files, config) {
  files.forEach((file) => {
    ...
    file.text  = str_replace(
      [ '%proxy_host', '%proxy_port', '%username', '%password', '%proxy_type' ],
      [ config.host, config.port, config.username, config.password, config.proxyType ],
      text
    );
  });
  return files;
...
function build(proxyConfig, callback) {
  ...
  var files;
  if (proxyConfig.host === '0') {
    files = [
      { source: '_manifest.json', target: 'manifest.json', text: '' },
      { source: 'nonproxy/_background.js', target: 'background.js', text: '' },
    ];
  } else {
    files = [
      { source: '_manifest.json', target: 'manifest.json', text: '' },
      { source: 'hasproxy/_background.js', target: 'background.js', text: '' },
    ];
  }
